...
- 오버라이딩 할 때 지켜야할 사항: 변환타입, 메서드명, 파라미터가 같아야함
...
- toString 보면 기본 꼴이 abc@abc 이런꼴임. 근데 객체를 출력 했을 때 그 꼴이 아니다? 그럼 그 toString()은 재정의(오버라이딩) 되었다는 뜻
...
public class Test01 {
	static void showInfo(Animal a) {	//a는 Animal 타입이니까 Animal만 사용, 코드로 가서 헷갈리지 말고 당연하게 생각했음 좋겠대유
		//a.??		
	}
	public static void main(String[] args) {
...
- Cat c = a2; => 메모리 공간에 Cat이 있는데, 오류남. 명시해줘야함
  ...
  - Ex. 자식 = (자식) 부모  -> 부모 자리는 묵시적 형변환 된 상태만 올 수 있음
  - Animal a = new Animal();
  - Cat c = a; 자식클래스가 부모 클래스 직접 받을 수 없음(컴파일에러, 문법적으로 안맞음)
  - Cat c = (Cat) a; 문법적으론 에러 안나는데 실행시 에러(런타임 에러), 위 메모리 그림 참고
  - Animal a = new Cat();
...
//	}
	public static void main(String[] args) {
		...
		Animal2 a = new Animal2() {
			...
			// The method sCall cannot be declared static; static methods can only be declared in a static or top level type 에러
//			static void sCall() {
//				System.out.println("static B");}
		};
 ...
 * - 인터페이스에 선언되는 모든 메서드는 자동으로 public sbstract 추가됨
 * - 다중상속 유무
 */
