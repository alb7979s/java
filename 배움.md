...
- 위 위의 예제에서 상속 관계 보면 Singer -> Person -> Object 이 관계임
...
![image-20210122104301684](C:\Users\20131\AppData\Roaming\Typora\typora-user-images\image-20210122104301684.png)

- 메모리가 이런식으로 올라가는데, 클래스 정보는 바뀌지 않으니 처음 생성될 때(처음 new 할 때) class loding은 한번만 됨
- 클래스 자체 정보는 Class Area(Method Area)에 올라감

![image-20210122104540833](C:\Users\20131\AppData\Roaming\Typora\typora-user-images\image-20210122104540833.png)
...
//		Sub04 s = Sub04.instance;
		...
		/*
		 Thread t = new Thread(new Runnable(){
		  ...
		  //이런식으로 짜면 여러개 동시 접근 가능 그래서 나중 되면 public static Sub04 이부분 synchronize 추가해주면 됨!!
		 */
	}
}
```
